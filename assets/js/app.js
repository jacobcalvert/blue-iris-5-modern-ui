(function () {
  ...
  async function refreshDashboard(initial = false) {
    ...
    try {
      ...
      renderAll();
      if (!initial) showToast("Dashboard refreshed", "Camera, alert, recording, and system data are up to date.");
    } catch (error) {
      if (error?.code === "session") {
  ...
  function renderCameraGrid() {
    ...
    el.cameraGrid.innerHTML = cameras.map((camera) => {
      ...
      return `
        <article class="camera-card${online ? "" : " is-offline"}" tabindex="0" role="button"
          ...
          <div class="camera-card__media">
            ...
            <div class="camera-card__top">
              <span class="status-chip ${statusClass}"><span></span>${escapeHtml(status)}</span>
              <div class="camera-card__badges">
                ${camera.audio ? `<span class="camera-card__badge" title="Audio">${icon("volume")}</span>` : ""}
                ${recording ? `<span class="camera-card__badge camera-card__badge--record" title="Recording">${icon("record")}</span>` : ""}
              </div>
            </div>
            ${online ? "" : `<div class="camera-card__offline">${icon("alert")}<span>No signal</span></div>`}
          </div>
          <div class="camera-card__footer">
            ...
            <div class="camera-card__stats">
              ...
              ${Number(camera.newalerts || 0) ? `<span>${Number(camera.newalerts)} new</span>` : `<span>${recording ? "REC" : "Ready"}</span>`}
            </div>
          </div>
        </article>
      `;
  ...
  function renderSystem() {
    ...
    const metrics = [
      ...
      { label: "Memory", value: status.ram || status.mem || "—", bar: memory, icon: "server", detail: status.memload ? `${status.memload} physical memory used` : "Blue Iris process" },
