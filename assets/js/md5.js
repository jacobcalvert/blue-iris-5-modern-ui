(function () {
  ...
  function blockAt(text, offset) {
    ...
    for (let i = 0; i < 64; i += 4) {
      block[i >> 2] =
        text.charCodeAt(offset + i) +
        (text.charCodeAt(offset + i + 1) << 8) +
        (text.charCodeAt(offset + i + 2) << 16) +
        (text.charCodeAt(offset + i + 3) << 24);
    }
    return block;
  }
  ...
  function hex(value) {
    ...
    let output = "";
    for (let i = 0; i < 4; i += 1) {
      output += chars[(value >> (i * 8 + 4)) & 15] + chars[(value >> (i * 8)) & 15];
    }
    return output;
  }

  window.biMd5 = function md5(input) {
    const text = unescape(encodeURIComponent(String(input)));
    const state = [1732584193, -271733879, -1732584194, 271733878];
    let offset;

    for (offset = 64; offset <= text.length; offset += 64) {
      cycle(state, blockAt(text, offset - 64));
    }

    const tail = new Array(16).fill(0);
    const remainder = text.slice(offset - 64);
